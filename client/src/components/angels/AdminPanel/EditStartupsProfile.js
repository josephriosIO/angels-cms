...
const EditStartupsProfile = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const user = await axios(
        `/api/profile/getStartups/profile/${props?.location?.startups?.id}`,
        {
          headers: {
  ...
  const handleSubmits = async event => {
    ...
    try {
      const token = await getTokenSilently();
      await axios.post(
        `/api/admin/update-startup/profile/${props?.location?.startups?.id}`,
        { form },
        {
          headers: {
            Authorization: `Bearer ${token}`,
...
export default EditStartupsProfile;
